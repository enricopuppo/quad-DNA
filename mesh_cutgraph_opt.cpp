...
void Mesh::populateArches()
{
    ...
    for (Node n : nodes)
    {
        ...
        do
        {
            hi = pivotCW(hi);
            Half h = halfs[hi];
            if (edges[h.ei].isCut && !isMarked(edge(h)))
                createArchFrom(h);
        } while (hi != start);
    }

    for (Arc &a : arcs)
        a.priority = evaluatePriority(a);
    // std::cout << "* arcs: " << arcs.size() << std::endl;
}

void Mesh::print(const Arc &a) const
{
    (*logFile) << "Arc from " << nodes[a.ni].vi << " (" << nodes[a.ni].valency << ")";
    (*logFile) << " to " << nodes[a.nj].vi << " (" << nodes[a.nj].valency << "): ";
}

void Mesh::print(const Node &n) const
{
    (*logFile) << "+node " << n.vi << ": arch-val=" << n.valency << ",edge-val=" << verts[n.vi].valency << "\n";
}
void Mesh::printNodes() const
{
    (*logFile) << "Nodes[" << nodes.size() << "]:\n";
    for (Node n : nodes)
        print(n);
}
