...
public:
    ...
    void initMotors(){
        ...
        for (Vert v:mesh.verts) {
            if (!v.isRegular()) spawnAround( v );
        }

        if (motors.empty()) {
            // no irreuglar? spawn around an arbitrary vert
            if (!mesh.verts.empty()) spawnAround( mesh.verts[0] );
        }
    ...
    Motor& nextAliveMotor(){
        ...
        do {
            ...
            if (motors[turn].isAlive()) return motors[turn];
        } while (initTurn != turn);
