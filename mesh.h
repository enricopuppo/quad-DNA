...
struct BoundingCube { glm::vec3 center; float radius; };
...
class Mesh
{
    ...
    template<class Type> bool isMarked(const Type &t) const { return t.mark==incrementalMark;}
    template<class Type> void mark(Type &t) { t.mark=incrementalMark;}
