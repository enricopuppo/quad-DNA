...
bool BasePair::isCanon() const
{
    ...
    if (base[0] > 4)
        return false;
    if (base[1] > 4)
        return false; // no baes beyond G ("go back")
    return true;
}
...
DNA reverseTranscription(const RNA &rna)
{
    ...
    if (genus != 0)
        std::cout << "DNA for Genus " << genus << " >0 ! Let's go!!!" << std::endl;
    ...
    for (int ci = 0;; ci = (ci + 1) % cn)
    {
        ...
        else
        {
            ...
            if (direction == DOWN)
            {
                ...
                if (!visited.empty())
                    visited.back().helixPos = dna.helix.size();
            }
            else
            {
                // UP => DOWN (an "armpit")
                if (!visited.empty())
                    dna.helix[visited.back().helixPos].openRounds++;
...
bool DNA::findMatchingOpenParOf(int &helixPos) const
{
    ...
    {
        helixPos--;
        if (helix[helixPos].closedRound)
            inside++;
        inside -= helix[helixPos].openRounds;
        if (helixPos == 0)
            inside--; // implict parentesis at the end
    } while (inside > 0);
    return (inside < 0);
}
...
RNA transcription(const DNA &d)
{
    ...
    for (int i = 0, c = 0; i < viRemap.size(); i++)
        viRemap[i] = (i != 0 && dna.helix[i % dna.helix.size()].portOut) ? (-1) : (c++);
    // for (int H : viRemap)
    //     std::cout << " " << H;
    // std::cout << std::endl;
    ...
    do
    {
        ...
        if (rna.chunks.size() == dna.helix.size() * 2)
            break;
        s = dna.step(s);
        // if (safety--<0) break;
    } while (1);

    for (BasePair b : dna.helix)
        rna.spanLenghts.push_back(b.len);
