...
class OutputObjFile{
    std::ofstream file;
public:
    OutputObjFile(const std::string& filename):file(filename) {
        std::cout << "OBJ file: " << filename << std::endl;
        if (!isOpen()) {
    ...
    void saveQuad(Quad q){
        file << "f";
        for (int w=0; w<4; w++) file  << " " << (q.vi[w]+1);
        file << "\n";
    }
    void saveQuad(Quad qa,Quad qb){
        file << "f";
        for (int w=0; w<4; w++) file  << " " << (qa.vi[w]+1) << "/" << (qb.vi[w]+1);
        file << "\n";
    }
...
class InputObjFile{
    std::ifstream file;
public:
    InputObjFile(const std::string& filename):file(filename) {
...
bool Mesh::importObj(const std::string& filename)
{
    ...
    InputObjFile file(filename);
    if (!file.isOpen()) return false;

    file.readVertsAndQuads(verts,quads);
    std::cout << "* "<< quads.size() << " quad faces and " << verts.size() <<  " verts."<< std::endl;

    populateHalfs();
    if (!isClosed()) std::cout << "WARNING mesh not closed!" << std::endl;
    ...
    std::cout << "* "<< countIrregulars() << " irregular vertices" << std::endl;

    return true;
}

bool Mesh::exportCutsAsObj(const std::string& filename) const{
    std::cout << "Saving cuts in ";
    OutputObjFile file(filename);
    if (!file.isOpen()) return false;

    const glm::vec3 smallDelta(0.001,0.00,0.001);
    for (Vert v:verts) file.savePos( v.pos );
    for (Vert v:verts) file.savePos( v.pos + smallDelta);

    for (Edge e:edges) {
        if (e.isCut) file.saveEdge( viOf(e), vjOf(e), verts.size() );
    }
    return true;
}

bool Mesh::exportObj(const std::string& filename) const{
    std::cout << "Saving mesh in ";
    OutputObjFile file(filename);
    if (!file.isOpen()) return false;

    for (Vert v:verts) file.savePos( v.pos );
    for (Quad q:quads) file.saveQuad( q );
...
bool Mesh::exportObj(const std::string& filename, const Mesh &uvMap) const{
    ...
    for (Vert v:verts) file.savePos( v.pos );
    for (Vert v:uvMap.verts) file.saveUv( v.pos );
    for (int qi=0; qi<nq; qi++) file.saveQuad( quads[qi], uvMap.quads[qi] );

    return true;
}

bool PrimaryStructure::exportAsObj(const std::string& filename) const{
    std::cout << "Saving 2D bundary in ";
    OutputObjFile file(filename);
    if (!file.isOpen()) return false;
    ...
    do {
        ...
        if (prev!=-1) file.saveEdge( prev, i , sequence.size() );
        prev = i;
        i = a.next;
